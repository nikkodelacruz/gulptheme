...
/**
 * This is a complete gravity forms hooks for:
 * Login 
 * Register 
 * Forgot Password 
 * Change Password 
...
function gform_custom_validation( $validation_result ) {
    ...
    elseif($form['id'] == 3){
        if ( rgpost("input_1") && !email_exists(rgpost("input_1")) ){
            $validation_result['is_valid'] = false;
            foreach( $form['fields'] as &$field ) {
                if($field->id == '1'){
                    $field->failed_validation = true;
    ...
    /* CHANGE PASSWORD FORM */
        ...
        if ( rgpost( 'input_2') && rgpost( 'input_1' ) != rgpost( 'input_2' ) ) {
            $id_arr[] = 2;
            $msg_arr[2] = "Password mismatched";
        }
        // -check password length
        if( rgpost( 'input_1') && strlen(rgpost( 'input_1')) <=5 ){
            $id_arr[] = 1;
            $msg_arr[1] = "Password must be atleast 6 characters long";
        }
